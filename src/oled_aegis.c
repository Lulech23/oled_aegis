...
// Screen saver fade animation timing
#define SCREEN_SAVER_FADE_STEP_MS			10
#define SCREEN_SAVER_FADE_DURATION_MS		150
#define SCREEN_SAVER_FADE_STEPS				(SCREEN_SAVER_FADE_DURATION_MS / SCREEN_SAVER_FADE_STEP_MS)
#define FULLSCREEN_ACTIVITY_OVERLAP_RATIO	0.99
...
void ApplyScreenSaverWindowOpacity(HWND hWnd, BYTE alpha) {
	if (!hWnd) return;
...
int GetMonitorIndexFromWindow(HWND hWnd) {
...
void StartScreenSaverFade(int monitorIndex, int fadeIn) {
	...
	int targetAlpha = (g_app.config.screenSaverOpacity * 255 + 50) / 100;
	targetAlpha = ClampInt(targetAlpha, 0, 255);

	g_monitorStates[monitorIndex].fadeTargetAlpha = targetAlpha;
	g_monitorStates[monitorIndex].fadeStep = fadeIn ? 0 : SCREEN_SAVER_FADE_STEPS;
	g_monitorStates[monitorIndex].fadeDirection = fadeIn ? 1 : -1;
	g_monitorStates[monitorIndex].currentAlpha = fadeIn ? 0 : targetAlpha;

	if (fadeIn) {
		ShowWindow(hWnd, SW_SHOW);
		ApplyScreenSaverWindowOpacity(hWnd, 0);
	} else {
		ApplyScreenSaverWindowOpacity(hWnd, targetAlpha);
	}

	SetTimer(hWnd, (UINT_PTR)hWnd, SCREEN_SAVER_FADE_STEP_MS, NULL);
}

void UpdateAllScreenSaverWindowOpacity() {
	int targetAlpha = (g_app.config.screenSaverOpacity * 255 + 50) / 100;
	targetAlpha = ClampInt(targetAlpha, 0, 255);
	...
	for (int i = 0; i < g_monitorCount; i++) {
		...
		if (hWnd) {
			ApplyScreenSaverWindowOpacity(hWnd, targetAlpha);
			g_monitorStates[i].fadeTargetAlpha = targetAlpha;
			g_monitorStates[i].currentAlpha = targetAlpha;
		}
	}
}
...
LRESULT CALLBACK MonitorWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		...
		case WM_TIMER:
			if ((UINT_PTR)wParam == (UINT_PTR)hWnd) {
				int monitorIndex = GetMonitorIndexFromWindow(hWnd);
				if (monitorIndex >= 0) {
					MonitorState* monitorState = &g_monitorStates[monitorIndex];
					int targetAlpha = monitorState->fadeTargetAlpha;

					if (monitorState->fadeDirection > 0) {
						monitorState->fadeStep++;
						if (monitorState->fadeStep > SCREEN_SAVER_FADE_STEPS) {
							monitorState->fadeStep = SCREEN_SAVER_FADE_STEPS;
						}

						BYTE alpha = (BYTE)((targetAlpha * monitorState->fadeStep) / SCREEN_SAVER_FADE_STEPS);
						ApplyScreenSaverWindowOpacity(hWnd, alpha);
						monitorState->currentAlpha = alpha;

						if (monitorState->fadeStep >= SCREEN_SAVER_FADE_STEPS) {
							KillTimer(hWnd, (UINT_PTR)hWnd);
							monitorState->fadeDirection = 0;
							monitorState->currentAlpha = targetAlpha;
						}
					} else if (monitorState->fadeDirection < 0) {
						monitorState->fadeStep--;
						if (monitorState->fadeStep < 0) {
							monitorState->fadeStep = 0;
						}
						...
						if (monitorState->fadeStep <= 0) {
							...
							ApplyScreenSaverWindowOpacity(hWnd, 0);
							monitorState->screenSaverActive = 0;
							monitorState->fadeDirection = 0;
							monitorState->currentAlpha = 0;
							...
							LogMessage("Screen saver window hidden on monitor %d after fade-out", monitorIndex);
						}
					}
				}
				return 0;
	...
	if (f) {
		...
		while (fgets(line, sizeof(line), f)) {
			...
			if (sscanf(line, "%299[^=]=%63s", key, value) == 2) {
				...
				} else if (strcmp(key, "screenSaverOpacity") == 0) {
					g_app.config.screenSaverOpacity = ClampInt(atoi(value), 25, 100);
				} else if (strcmp(key, "timeoutEnabled") == 0) {
					g_app.config.timeoutEnabled = ClampInt(atoi(value), 0, 1);
...
void SaveConfig() {
	...
	if (f) {
		...
		fprintf(f, "screenSaverOpacity=%d\n", g_app.config.screenSaverOpacity);
		fprintf(f, "timeoutEnabled=%d\n", g_app.config.timeoutEnabled);
...
void HideScreenSaverOnMonitor(int monitorIndex) {
	...
	if (g_monitorStates[monitorIndex].screenSaverActive) {
		g_monitorStates[monitorIndex].fadeStep = SCREEN_SAVER_FADE_STEPS;
		g_monitorStates[monitorIndex].fadeDirection = -1;
		StartScreenSaverFade(monitorIndex, 0);
		LogMessage("Screen saver fade-out started on monitor %d", monitorIndex);
	} else {
		ShowWindow(hWnd, SW_HIDE);
		ApplyScreenSaverWindowOpacity(hWnd, 0);
...
int IsShellOverlayClass(const char* className) {
	if (!className || className[0] == '\0') return 0;

	if (_stricmp(className, "Progman") == 0 ||
		_stricmp(className, "WorkerW") == 0 ||
		_stricmp(className, "Shell_TrayWnd") == 0 ||
		_stricmp(className, "Windows.UI.Core.CoreWindow") == 0 ||
		_stricmp(className, "XamlExplorerHostIslandWindow") == 0) {
		return 1;
	}

	if (strstr(className, "Shell_") != NULL) {
		return 1;
	}
...
void UpdateFullscreenMonitorActivity(int fullscreenOnMonitor[MAX_MONITOR_COUNT], time_t now) {
	static DWORD lastLoggedMask = (DWORD)-1;

	for (int i = 0; i < MAX_MONITOR_COUNT; i++) {
		fullscreenOnMonitor[i] = 0;
	}
	...
	if (!hFg) {
		...
		if (mask != lastLoggedMask) {
			LogMessage("Fullscreen monitor activity: mask=0x%08X", mask);
			lastLoggedMask = mask;
		}
		return;
	}
	...
	if (!IsWindowVisible(hFg) || IsIconic(hFg) || IsWindowCloakedCompat(hFg)) {
		DWORD mask = 0;
		if (mask != lastLoggedMask) {
			LogMessage("Fullscreen monitor activity: mask=0x%08X", mask);
			lastLoggedMask = mask;
	...
	if (IsShellOverlayClass(className)) {
		DWORD mask = 0;
		if (mask != lastLoggedMask) {
			LogMessage("Fullscreen monitor activity: mask=0x%08X", mask);
			lastLoggedMask = mask;
	...
	RECT rect;
	if (!GetVisibleWindowRect(hFg, &rect)) {
		DWORD mask = 0;
		if (mask != lastLoggedMask) {
			LogMessage("Fullscreen monitor activity: mask=0x%08X", mask);
			lastLoggedMask = mask;
	...
	for (int i = 0; i < g_monitorCount; i++) {
		LONGLONG intersectionArea = RectIntersectionArea(&rect, &g_monitors[i].rect);
		if (intersectionArea <= 0) continue;

		LONGLONG monitorArea = RectArea(&g_monitors[i].rect);
		if (monitorArea <= 0) continue;

		double overlapRatio = (double)intersectionArea / (double)monitorArea;
	...
	if (matchedMonitor >= 0 && bestOverlapRatio >= FULLSCREEN_ACTIVITY_OVERLAP_RATIO) {
		fullscreenOnMonitor[matchedMonitor] = 1;
		g_monitorStates[matchedMonitor].lastInputTime = now;

		if (g_monitorStates[matchedMonitor].screenSaverActive) {
			LogMessage("Fullscreen activity detected on monitor %d, hiding active screen saver", matchedMonitor);
			HideScreenSaverOnMonitor(matchedMonitor);
		}
	}

	DWORD mask = 0;
	for (int i = 0; i < g_monitorCount && i < 32; i++) {
		if (fullscreenOnMonitor[i]) {
			mask |= (1u << i);
		}
	}
...
void ShowScreenSaverOnMonitor(int monitorIndex, int isManual) {
	if (monitorIndex < 0 || monitorIndex >= g_monitorCount) return;
	if (!g_app.config.timeoutEnabled) {
		LogMessage("Timeout disabled, skipping screen saver activation on monitor %d", monitorIndex);
		return;
	}
	...
	if (hWnd) {
		...
		g_monitorStates[monitorIndex].fadeStep = 0;
		g_monitorStates[monitorIndex].fadeDirection = 1;
		g_monitorStates[monitorIndex].fadeTargetAlpha = (g_app.config.screenSaverOpacity * 255 + 50) / 100;
		StartScreenSaverFade(monitorIndex, 1);
		LogMessage("Screen saver fade-in started on monitor %d (reused)", monitorIndex);
	} else {
		...
		if (hWnd) {
			...
			g_monitorStates[monitorIndex].fadeTargetAlpha = (g_app.config.screenSaverOpacity * 255 + 50) / 100;
			StartScreenSaverFade(monitorIndex, 1);
			LogMessage("Screen saver fade-in started on monitor %d", monitorIndex);
		}
	}
...
void ShowScreenSaver(int isManual) {
	if (!g_app.config.timeoutEnabled) {
		LogMessage("Timeout disabled, skipping global screen saver activation");
		return;
	}
...
LRESULT CALLBACK SettingsDialogProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch (message) {
		case WM_DPICHANGED:
			g_settingsDpi = HIWORD(wParam);
			// Optionally, resize/reposition controls here for DPI change
			break;
...
void ShowSettingsDialog() {
	...
	if (g_hSettingsDialog) {
		...
		icex.dwICC = ICC_UPDOWN_CLASS | ICC_BAR_CLASSES;
		InitCommonControlsEx(&icex);
		...
		// Dynamically determine font height for DPI
		int fontHeight = ScaleDPI(20);
		if (g_hSettingsFont) {
			HDC hdcFont = GetDC(g_hSettingsDialog);
			HFONT hOldFont = (HFONT)SelectObject(hdcFont, g_hSettingsFont);
			TEXTMETRIC tm;
			if (GetTextMetrics(hdcFont, &tm)) {
				fontHeight = tm.tmHeight;
			}
			SelectObject(hdcFont, hOldFont);
			ReleaseDC(g_hSettingsDialog, hdcFont);
		}
		int controlHeight = fontHeight + 8; // 8px padding for clarity
		int rowHeight = controlHeight + 5;  // 5px between rows
		int buttonHeight = controlHeight + 6; // Slightly taller for buttons
		...
		HWND hOpacityLabel = CreateWindowA("STATIC", "Screen Saver Opacity (%):",
					 WS_CHILD | WS_VISIBLE,
					 margin, y, labelWidth, controlHeight, g_hSettingsDialog, NULL, hMod, NULL);
		HWND hOpacitySlider = CreateWindowExA(0, TRACKBAR_CLASS, "",
					 WS_CHILD | WS_VISIBLE | TBS_AUTOTICKS,
					 margin + labelWidth, y, checkboxWidth - labelWidth, controlHeight + ScaleDPI(10),
					 g_hSettingsDialog, (HMENU)IDC_OPACITY_SLIDER, hMod, NULL);
		SendMessage(hOpacitySlider, TBM_SETRANGE, TRUE, MAKELONG(25, 100));
		SendMessage(hOpacitySlider, TBM_SETTICFREQ, 5, 0);
		y += rowHeight + ScaleDPI(5);

		HWND hVideoCheck = CreateWindowA("BUTTON", "Enable Media Playback Detection",
		...
		HWND hConfigBtn = CreateWindowA("BUTTON", "Open Config",
					 WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
		...
		if (g_hSettingsFont) {
			...
			SendMessageA(hOpacityLabel, WM_SETFONT, (WPARAM)g_hSettingsFont, TRUE);
			SendMessageA(hOpacitySlider, WM_SETFONT, (WPARAM)g_hSettingsFont, TRUE);
		...
		AddTooltip(g_hSettingsDialog, hOpacitySlider,
				   "Adjust the overall opacity of the screen saver overlay from 25% to 100%.");
		...
		SendMessage(GetDlgItem(g_hSettingsDialog, IDC_OPACITY_SLIDER), TBM_SETPOS, TRUE, g_app.config.screenSaverOpacity);
...
void ApplySettings(HWND hWnd) {
	...
	int oldOpacity = g_app.config.screenSaverOpacity;
	int newOpacity = (int)SendMessage(GetDlgItem(hWnd, IDC_OPACITY_SLIDER), TBM_GETPOS, 0, 0);
	newOpacity = ClampInt(newOpacity, 25, 100);
	g_app.config.screenSaverOpacity = newOpacity;
	...
	UpdateAllScreenSaverWindowOpacity();
	SaveConfig();
	UpdateStartupRegistry();

	LogMessage("Settings applied: timeout %ds->%ds, interval %dms->%dms, media %d->%d, debug %d->%d, startup %d->%d, perMonitor %d->%d, pixelShift %dpx, opacity %d%%->%d%%",
			 ...
			 g_app.config.pixelShiftCompensation,
			 oldOpacity, g_app.config.screenSaverOpacity);
...
void UpdateTrayIcon(int active) {
	g_app.nid.hIcon = g_app.config.timeoutEnabled ? g_hIconActive : g_hIconInactive;
	if (!g_app.config.timeoutEnabled) {
		lstrcpyA(g_app.nid.szTip, "OLED Aegis - Inactive");
	} else {
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch (message) {
		case WM_CREATE:
			...
			g_app.config.screenSaverOpacity = 100;
			g_app.config.timeoutEnabled = 1;
			for (int i = 0; i < MAX_MONITOR_COUNT; i++) {
			...
			for (int i = 0; i < g_monitorCount; i++) {
				...
				g_monitorStates[i].enabled = g_app.config.monitorsEnabled[i];
				g_monitorStates[i].hScreenSaverWnd = NULL;
				g_monitorStates[i].fadeStep = 0;
				g_monitorStates[i].fadeDirection = 0;
				g_monitorStates[i].fadeTargetAlpha = (g_app.config.screenSaverOpacity * 255 + 50) / 100;
				g_monitorStates[i].currentAlpha = 0;
			}
			...
			LogMessage("Application started. Timeout: %ds, Media: %d, Debug: %d, Opacity: %d%%, TimeoutEnabled: %d",
					 g_app.config.idleTimeout, g_app.config.mediaDetectionEnabled, g_app.config.debugMode,
					 g_app.config.screenSaverOpacity, g_app.config.timeoutEnabled);
		...
		case WM_TIMER:
			if (wParam == TIMER_IDLE_CHECK) {
				if (!g_app.config.timeoutEnabled) {
					if (IsAnyMonitorActive()) {
						LogMessage("Timeout disabled - hiding active screen saver windows");
						HideScreenSaver();
					}
					UpdateTrayIcon(0);
					break;
				...
				if (g_app.config.perMonitorInputDetection) {
					...
					time_t now = time(NULL);
					int fullscreenOnMonitor[MAX_MONITOR_COUNT] = {0};
					UpdateFullscreenMonitorActivity(fullscreenOnMonitor, now);
		...
		case WM_TRAYICON:
			if (lParam == WM_RBUTTONUP) {
				...
				AppendMenuA(hMenu, MF_STRING | (g_app.config.mediaDetectionEnabled ? MF_CHECKED : 0), IDM_TOGGLE_MEDIA_DETECTION, "Enable Media Playback Detection");
				...
				if (g_hSettingsDialog) {
					...
					g_app.config.timeoutEnabled = !g_app.config.timeoutEnabled;
					SaveConfig();
					LogMessage("User: Toggled timeout to %s", g_app.config.timeoutEnabled ? "enabled" : "disabled");
		...
		case WM_COMMAND:
			switch (LOWORD(wParam)) {
				case IDM_TOGGLE_MEDIA_DETECTION:
					g_app.config.mediaDetectionEnabled = !g_app.config.mediaDetectionEnabled;
					SaveConfig();
					LogMessage("User: Toggled media playback detection to %d", g_app.config.mediaDetectionEnabled);
					UpdateTrayIcon(IsAnyMonitorActive() ? 1 : 0);
					break;
				case IDM_SETTINGS:
